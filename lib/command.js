...
const Command = new Class({
  ...
  _parseArgvAndOptions: function () {
    ...
    var errArray = [], index = 0, len = this._tokens.length;
    //从 index=1 开始
    while ((++index) < len) {
      ...
      if (token.type === Token.TYPE_OPTION_NAME) { //如果是一个 options
        ...
        if (utils.isNull(option)) {
          errArray.push('Invalid option: ' + token.value);
          continue;
        }
        ...
        if (argument && !argument.testValue(token.value)) {
          errArray.push('Invalid Argument: ' + token.value);
          continue;
        }
    ...
    return errArray.length > 0 ?
      new Error(errArray.join(os.EOL)) : null;
  },
