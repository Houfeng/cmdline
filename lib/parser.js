...
const Parser = new Class({
   ...
   * 定义子命令处理函数
   **/
  command: function (names, handler, strict) {
  ...
  _findHandlers: function () {
    var handlers = this._handlers.filter(function (item) {
      ...
      return (utils.isNull(pattern.argc) || pattern.argc == this.argc) &&
        ...
        (utils.isNull(pattern.command) || (pattern.commandStrict ? !this._parseInjectArguments(item.handler)
          .some(function (value) {
            return utils.isNull(value);
          }, this) : pattern.command.test(this._command)))
    }, this);
